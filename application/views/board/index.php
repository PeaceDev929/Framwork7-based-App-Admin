<div class="row">
    ...
    <div class="col-md-12" style="padding-top: 20px;">
        <table id="table-content" class="table table-bordered table-hover">
            <thead>
            <tr>
                <th class="center title_td" width="110px">Opening date</th>
                <th class="center title_td">Board List</th>
                <th class="center title_td">NickName</th>
...
<script>
    ...
    $(function () {
        ...
        table = eTable.dataTable({
            ...
            "ajax": { // define ajax settings
                "url": "<?=site_url('Board/getTableData')?>", // ajax URL
                "type": "POST",
                "beforeSend": function () {
                },
                "data": function (data) {
            ...
            "createdRow": function (row, data, dataIndex) {
                $('td:eq(3)', row).css("padding", "0");
                $('td:eq(3)', row).html('<button class="btn btn-primary" onclick="deleteItem(' + data["uid"] + ')">Delete</button>');
            },
    ...
    function deleteItem(uid) {
        showConfirmDlg("Are you sure you want to delete the post?", function () {
            $.ajax({
                type: 'post',
                url: '<?=site_url("Board/deleteItem")?>',
                data: {
                    uid: uid
                },
